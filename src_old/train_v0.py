...
def launch_processes(global_model, optimizer):
    ...
    _, env_params = get_params()

    processes = []
    for index in range(opt.num_processes):
        if index == 0:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer, env_params, True))
        else:
            process = mp.Process(target=local_train, args=(index, opt, global_model, optimizer, env_params))
        process.start()
        processes.append(process)

    process = mp.Process(target=local_test, args=(opt.num_processes, opt, global_model, env_params))
    process.start()
    processes.append(process)
