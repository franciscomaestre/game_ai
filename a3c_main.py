...
from models.shallow import Actor as ShallowActor
from models.shallow import DiscreteActor as ShallowDiscreteActor
from models.shallow import Critic as ShallowCritic
from models.deep import Actor as DeepActor
from models.deep import DiscreteActor as DeepDiscreteActor
from models.deep import Critic as DeepCritic
...
def get_args():
    ...
    parser.add_argument("--env_name", help="Name of the Gym environment", type=str, default="SpaceInvaders-v4")
    parser.add_argument("--env_params", help="Name of the Parameters environment. It could be super_mario or atari", type=str, default="atari")
    parser.add_argument("--test", help="Tests a saved Agent model", type=int, default=0)
    parser.add_argument("--render", help="Whether to render the environment to the display. Default=False", type=int, default=0)
    parser.add_argument("--gpu-id", help="GPU device ID to use. Default:0", type=int, default=0)
...
summary_file_path = params_manager.get_agent_params()['summary_file_path_prefix'] + args.env_name + "_" + datetime.now().strftime("%y-%m-%d-%H-%M")
...
def main():
    ...
    agent_params, env_params = _get_params()

    env_params["env_name"] = args.env_name
    env_params["video"] = args.render

    agent_params["test"] = args.test
    agent_params["render"] = args.render
    
    manager = mp.Manager()
    shared_state = manager.dict()

    if not args.test:
        _train(agent_params, env_params, manager, shared_state)
    else:
        _test(agent_params, env_params, manager, shared_state)

def _train(agent_params, env_params, manager, shared_state):
        agent_procs =[DeepActorCriticAgent(id, args.env_name, agent_params, shared_state, env_params) for id in range(agent_params["num_agents"])]
        [p.start() for p in agent_procs]
        [p.join() for p in agent_procs]

def _test(agent_params, env_params, manager, shared_state):
        test_agent_proc = DeepActorCriticAgent(0, args.env_name, agent_params, shared_state, env_params)
...
def _init_seed():
    ...
    np.random.seed(seed)
    if torch.cuda.is_available() and params_manager.get_agent_params()['use_cuda']:
...
class DeepActorCriticAgent(mp.Process):
    ...
    def save(self):
        model_file_name = self.params["model_dir"] + "Async-A3C_" + self.env_name + ".ptm"
    ...
    def run(self):
        ...
        self.env = make_train_env(self.env_name, self.env_conf)
        ...
        if self.actor_name == "global":
            ...
            if self.params['load_trained_model']:
                try:
                    ...
                    if self.params["test"]:  # Test a saved model
                        print("No saved model found. Waiting a minute...")
                        time.sleep(60)
                        return self.run()
                    else:
